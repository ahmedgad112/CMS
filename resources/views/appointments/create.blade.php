            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('appointment_type') is-invalid @enderror" 
                        ...
                        <option value="checkup" {{ old('appointment_type') == 'checkup' ? 'selected' : '' }}>كشف</option>
                        <option value="consultation" {{ old('appointment_type') == 'consultation' ? 'selected' : '' }}>استشارة</option>
                    </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const allClinicOptions = Array.from(clinicSelect.options).map(o => ({
        value: o.value,
        text: o.textContent,
        selected: o.selected,
    }));
    const previouslySelectedClinic = clinicSelect.value;

    function filterClinicsByDoctor(doctorId) {
        if (!doctorId) {
            clinicSelect.innerHTML = '';
            allClinicOptions.forEach(o => {
                const opt = document.createElement('option');
                opt.value = o.value;
                opt.textContent = o.text;
                if (o.value === previouslySelectedClinic) opt.selected = true;
        ...
        fetch(`/api/doctors/${doctorId}/clinics`)
            ...
            .then(data => {
                ...
                data.clinics.forEach(c => {
                    ...
                    if (String(c.id) === String(previouslySelectedClinic)) opt.selected = true;
                    clinicSelect.appendChild(opt);
                });

                // Auto-select if only one clinic
                if (data.clinics.length === 1 && !previouslySelectedClinic) {
