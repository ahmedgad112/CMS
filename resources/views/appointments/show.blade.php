...
<div class="card mb-4 border-0 shadow-sm">
    <div class="card-body py-3">
        <div class="d-flex flex-column flex-sm-row flex-wrap align-items-stretch align-items-sm-center gap-2 justify-content-sm-between">
            <span class="text-muted small align-self-center mb-1 mb-sm-0">
                <i class="fas fa-hand-pointer me-1"></i> إجراءات الموعد
            </span>
            <div class="d-flex flex-wrap gap-2 justify-content-sm-end">
                @if($appointment->status === 'pending')
                <form action="{{ route('appointments.update', $appointment->id) }}" method="POST" class="d-inline flex-grow-1 flex-sm-grow-0">
                    @csrf
                    @method('PUT')
                ...
                <a href="{{ route('appointments.edit', $appointment->id) }}" class="btn btn-primary flex-grow-1 flex-sm-grow-0">
                    <i class="fas fa-edit me-1"></i> تعديل
                </a>
                @endif

                @if(in_array($appointment->status, ['pending', 'confirmed'], true))
                <form action="{{ route('appointments.update', $appointment->id) }}" method="POST" class="d-inline flex-grow-1 flex-sm-grow-0" onsubmit="return confirm('هل تريد إلغاء هذا الموعد؟');">
                    @csrf
                    @method('PUT')
            ...
            <div class="card-body p-4">
                <div class="d-flex flex-column gap-3">
                    ...
                    <form action="{{ route('appointments.update', $appointment->id) }}" method="POST" class="d-inline">
                        ...
                        <input type="hidden" name="appointment_type" value="{{ $appointment->appointment_type ?? 'checkup' }}">
                    ...
                    <form action="{{ route('appointments.destroy', $appointment->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="quick-action-card w-100 text-start border-0 p-3" style="background: white; cursor: pointer; border: 2px solid #dc3545 !important;" onclick="return confirm('هل أنت متأكد من حذف هذا الموعد نهائياً من النظام؟');">
                            <div class="quick-action-icon bg-danger">
                                <i class="fas fa-trash"></i>
                            </div>
                            <div class="quick-action-content">
                                <h6 class="mb-1">حذف نهائي</h6>
                                <small class="text-muted">إزالة السجل من النظام</small>
                            </div>
                            <div class="quick-action-arrow">
                                <i class="fas fa-arrow-left"></i>
                            </div>
