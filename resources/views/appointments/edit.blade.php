            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <div class="d-flex gap-2 mb-2">
                        ...
                        <a href="{{ route('patients.create', ['return_to' => route('appointments.edit', $appointment->id)]) }}" 
                    ...
                    <small class="form-text text-muted">
                        ...
                        ابدأ بكتابة رقم الهاتف أو الاسم للبحث، أو اضغط على "مريض جديد" لإضافة مريض جديد
                    </small>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('doctor_id') is-invalid @enderror" 
                        ...
                        @foreach($doctors as $doctor)
                            @php
                                $departmentName = $doctor->department && is_object($doctor->department) ? $doctor->department->name : '';
                                $specializationName = null;
                                if ($doctor->specialization_id) {
                                    if ($doctor->relationLoaded('specialization') && $doctor->specialization && is_object($doctor->specialization)) {
                                        $specializationName = $doctor->specialization->name;
                                    }
                                }
                                if (!$specializationName && !empty($doctor->specialization) && is_string($doctor->specialization)) {
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <input type="hidden" id="appointment_date" name="appointment_date" value="{{ old('appointment_date', $appointment->appointment_date->format('Y-m-d')) }}" required>
                    <input type="hidden" id="appointment_time" name="appointment_time" value="{{ old('appointment_time', $appointment->appointment_date->format('H:i')) }}" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if(old('patient_id', $appointment->patient_id))
        const selectedPatientId = {{ old('patient_id', $appointment->patient_id) }};
    ...
    function applyInitialSelection() {
        ...
        document.querySelectorAll('.time-slot-btn').forEach(btn => {
            if (btn.dataset.date === dateVal && btn.dataset.time === timeVal) {
                btn.classList.remove('btn-outline-primary');
                btn.classList.add('btn-primary');
                selectedSlotText.textContent = btn.dataset.display;
                selectedSlotInfo.style.display = 'block';
                found = true;
            }
        });
        if (!found) {
            selectedSlotText.textContent = `${dateVal} — ${timeVal}`;
            selectedSlotInfo.style.display = 'block';
        }
    }
    ...
    function generateScheduleTable(doctorId) {
        ...
        if (availableDays.length === 0) {
            scheduleContainer.innerHTML = '<div class="alert alert-warning mb-0"><i class="fas fa-exclamation-triangle me-2"></i>لا توجد أيام متاحة لهذا الطبيب</div>';
            scheduleContainer.style.display = 'block';
            scheduleTableContainer.style.display = 'none';
            applyInitialSelection();
            return;
        ...
        if (scheduleData.length === 0) {
            scheduleTbody.innerHTML = '<tr><td colspan="2" class="text-center text-muted">لا توجد مواعيد متاحة في الأيام القادمة</td></tr>';
        }
        applyInitialSelection();
    }
    ...
    function loadDoctorSchedules(doctorId) {
        ...
        fetch(`/api/doctor/${doctorId}/schedules`)
            .then(response => response.json())
            .then(data => {
            ...
            .catch(error => {
                console.error('Error:', error);
                doctorSchedules = [];
                scheduleContainer.innerHTML = '<div class="alert alert-danger mb-0"><i class="fas fa-exclamation-triangle me-2"></i>حدث خطأ في جلب المواعيد</div>';
                scheduleContainer.style.display = 'block';
                scheduleTableContainer.style.display = 'none';
                applyInitialSelection();
            });
